...
Status: derived entirely from static analysis of a real Nextion Editor
project file (`ver21234` internally, a NanoEls H5 lathe UI). Nothing here
...
This document describes the format this toolkit's [`hmi`](../../src/hmi.rs)
module implements. See [targets.md](../targets.md) for why this toolkit
only supports one physical display, and [spec-format.md](../spec-format.md)
for the YAML spec `compile` consumes on top of these primitives.
...
something computed from content size. [`find_payload_start`](../../src/hmi.rs)
doesn't hardcode this offset — it scans past the mirrored directory blocks
for the first block that's mostly non-zero, so it works on `.HMI` files
with a different amount of reserved/erased space.

## 2. Directory (block 0/1) **[confirmed, partially]**

Starts at offset 0, ends once the file goes all-zero (~0x3C0 bytes used in
the reference project). It's a flat list of resource entries, one per
compiled asset. Names seen: `main.HMI`, `Program.s` (the global startup
script — plain readable Nextion Instruction Set text), and per
...
verified against all entries, and the exact meaning of the trailing
...
adding/removing components — see §5. This toolkit does not implement the
directory at all: it locates the payload heuristically (§1) and never
writes to the directory blocks.
...
16-byte name pattern" scanner (this toolkit's [`scan_records`](../../src/hmi.rs)
included) has no way to know where such a blob ends except "wherever the
next recognizable name pattern happens to reappear" — so it will
occasionally attribute several KB of an embedded PNG to whatever
attribute preceded it. This doesn't corrupt anything on disk, it's purely
a limitation of a scanner that doesn't know the declared length up front.
A byte-accurate tool needs the *actual* length either from the directory
(§2) or from a proper per-attribute length prefix we haven't found yet for
the blob types — plain string/int attributes are unaffected and decode
perfectly. This is why [`AttrValue::Blob`](../../src/hmi.rs) exists as a
distinct case from `Str`/`Int`: it's the toolkit telling you "this value's
boundaries are best-effort."

## 4. What this enables today

- **Decode**: turn any `.HMI`'s component tables into a readable,
  diffable, version-controllable text form (YAML). Solid — this is what
  `nxtft hmi-decode` does.
- **Patch in place**: change the *value* of an existing attribute — a
  ...
  around it. This is what `nxtft hmi-patch` does. **Not independently
  verified against the real Nextion Editor** — see §6.
...
- **Adding or removing a component**, or changing a string's length
  ...
  unsupported, and this toolkit refuses them (`hmi::patch_attr` only
  accepts same-length overwrites).
- We have not looked at how the `.tft` compiled-firmware format's page
  layout encodes component count at all. Getting from edited `.HMI` →
  `.tft` still goes through Nextion Editor's own (closed,
  unreverse-engineered) compiler — see
  [nextion-tft-format.md](nextion-tft-format.md) §6 for the workaround
  this toolkit uses instead.

## 6. Recommended validation loop before trusting this for real work

1. Pick one harmless attribute (e.g. a button's background color, or its
   `txt` value with a same-length replacement).
2. Patch it with `nxtft hmi-patch`.
...
4. Only after that, attempt the harder case: add one new component (copy
   ...
   keeps the file valid). This toolkit doesn't attempt this at all today.
